...
function composeLockExtendTimeBytes(tokenId: bigint, duration: bigint) {
  const { abi } = Ve;
...
export default function useExtendLockTime({
  duration,
  tokenId,
  onSuccess,
  onError,
  ...
  const execute = useCallback(
    () =>
      sendTransaction({
        data: composeLockExtendTimeBytes(tokenId, duration),
        to: escrow,
      }),
    [sendTransaction, tokenId, duration, escrow]
  );
